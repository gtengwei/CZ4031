...
private:
  ...
  // add in hardcoded values for testing
  void staticInsert();
...
public:
  ...
  void insert(std::tuple<void*, size_t> address, float key);

  // Inserts a record into a linked list. Returns the address of the new linked list head (if any).
  std::tuple<void*, size_t> insertLL(std::tuple<void*, size_t> LLHead, std::tuple<void*, size_t> address, float key);
  ...
  void displayLL(std::tuple<void*, size_t> LLHeadAddress);
  ...
  void removeLL(std::tuple<void*, size_t> LLHeadAddress);
